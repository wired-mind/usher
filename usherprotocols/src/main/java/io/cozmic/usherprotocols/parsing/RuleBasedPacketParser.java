...
public class RuleBasedPacketParser implements Handler<Buffer> {
    ...
    public static RuleBasedPacketParser fromConfig(JsonObject config, Handler<Buffer> handler) {
        if (config == null || handler == null) {
            throw new IllegalArgumentException();
        }
        final RuleBasedPacketParser ruleBasedPacketParser = new RuleBasedPacketParser(config, handler);
        return ruleBasedPacketParser;
    }



    private RuleBasedPacketParser(JsonObject config, final Handler<Buffer> handler) {
    ...
    private static class TypeMapRule extends Rule {
        ...
        public TypeMapRule(JsonObject config) {
            ...
            for (Object objEntry : mapEntries) {
                final JsonObject entry = new JsonObject((Map<String, Object>) objEntry);
